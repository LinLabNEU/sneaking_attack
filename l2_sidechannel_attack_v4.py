...
class LADMML2re:
    ...
    def gradient_descent(self, sess, model):
        ...
        # modify kernal or bias weights, or kernal and bias weights
        ...
        wei[0:self.s] = 200.0 * wei[0:self.s]
        loss1 = loss1 * wei
    ...
    #    model.model.layers[13].kernel = model.model.layers[13].kernel - tdelt
        ...
        def doit(imgs, labs, z):
            ...
            scores, delt_grad_kernel, delt_grad_bias = sess.run([output, grad_tdelt_kernel, grad_tdelt_bias])

            delt_gradss = np.hstack((np.reshape(delt_grad_kernel, (-1)), np.reshape(delt_grad_bias, (-1))))
            return scores, np.array(delt_gradss)
